...

{
  ...
  2006-04-14 -
               ...
               IMPROVE 1 - Add classes and methods to work with the pHYs chunk
                           (including TPNGImage.DrawUsingPixelInformation)
  ...
  2003-03-04 - * NEW * Property Pixels for direct access to pixels
               * IMPROVED * Palette property (TPNGImage) (read only)
...
{$DEFINE RegisterGraphic}        //Registers TPNGImage to use with TPicture
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        {Copies the new data}
        CopyMemory(Ptr(DWORD(Output) + DWORD(OutputSize) - total_out),
          @Buffer, total_out);
...
begin
  ...
  begin
    ...
    begin
      {Writes this IDAT chunk}
      WriteIDAT(fStream, Data, DWORD(Owner.MaxIdatSize) - avail_out);
      {Re-update buffer}
      next_out := Data;
      avail_out := Owner.MaxIdatSize;
    end;

    if avail_out < DWORD(Owner.MaxIdatSize) then
      {Writes final IDAT}
      WriteIDAT(fStream, Data, DWORD(Owner.MaxIdatSize) - avail_out);
