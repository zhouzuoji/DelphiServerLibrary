...
*    2009.04.14  added overloaded string routines for RawByteString and                             *
*                  UnicodeString                                                                 *
...
type
  ...
  private
    ...
    FComment : RawByteString;
    FDateTime: TDateTime;

    FTrailer : TGZTrailer;
  public
    constructor Create(dest: TStream; const fileName, comment: RawByteString;
...
function  GZCompressStr(const s: RawByteString; const fileName,
  comment: RawByteString; dateTime: TDateTime): RawByteString; overload;

procedure GZCompressString(var result: RawByteString; const s: RawByteString;
  const fileName, comment: RawByteString; dateTime: TDateTime); overload;
...
function  GZCompressStr(const s: RawByteString): RawByteString; overload;
...
function  GZDecompressStr(const s: RawByteString; var fileName,
  comment: RawByteString; var dateTime: TDateTime): RawByteString; overload;

procedure GZDecompressString(var result: RawByteString; const s: RawByteString;
  var fileName, comment: RawByteString; var dateTime: TDateTime); overload;

{$ifdef Version6Plus}
procedure GZDecompressString(var result: UnicodeString;
  const s: RawByteString; var fileName, comment: RawByteString;
  var dateTime: TDateTime); overload;
{$endif}

function  GZDecompressStr(const s: RawByteString): RawByteString; overload;
...
procedure GZCompressStream(inStream, outStream: TStream; const fileName,
  comment: RawByteString; dateTime: TDateTime); overload;
...
procedure GZDecompressStream(inStream, outStream: TStream; var fileName,
  comment: RawByteString; var dateTime: TDateTime); overload;
...
function  GZDecompressStreamSize(inStream: TStream; var fileName,
  comment: RawByteString; var dateTime: TDateTime): Longint; overload;
...
procedure GZCompressFile(const inFileName, outFileName: String;
  const fileName, comment: RawByteString); overload;

procedure GZCompressFile(const inFileName, outFileName: String;
  const comment: RawByteString); overload;
...
procedure GZDecompressFile(const inFileName, outFolder: String;
  var comment: RawByteString); overload;

procedure GZDecompressFile(const inFileName, outFolder: String); overload;

function GzipCompress(const buf; len: Integer): RawByteString; overload;
function GzipCompress(const s: RawByteString): RawByteString; overload;
function GzipDecompress(const s: RawByteString): RawByteString; overload;
function GzipDecompress(const buf; len: Integer): RawByteString; overload;
...
procedure GZInitializeCompressString(var result: RawByteString;
  const fileName, comment: RawByteString; dateTime: TDateTime);
var
  header: PGZHeader;
begin
...
procedure GZInitializeDecompressString(const s: RawByteString; var fileName,
  comment: RawByteString; var dateTime: TDateTime; var index: Integer);
var
  header  : PGZHeader;
  maxIndex: Integer;
  endIndex: Integer;
...
function GZCompressStr(const s: RawByteString; const fileName,
  comment: RawByteString; dateTime: TDateTime): RawByteString;
begin
...
function GZCompressStr(const s: RawByteString): RawByteString;
begin
...
function GZDecompressStr(const s: RawByteString; var fileName,
  comment: RawByteString; var dateTime: TDateTime): RawByteString;
begin
  GZDecompressString(result, s, fileName, comment, dateTime);
end;

procedure GZDecompressString(var result: RawByteString; const s: RawByteString;
  var fileName, comment: RawByteString; var dateTime: TDateTime);
var
  index: Integer;
  crc  : Longint;
  size : Integer;
...
function GZDecompressStr(const s: RawByteString): RawByteString;
begin
...
procedure GZCompressStream(inStream, outStream: TStream; const fileName,
  comment: RawByteString; dateTime: TDateTime);
const
  bufferSize = 32768;
var
  header    : TGZHeader;
...
procedure GZDecompressStream(inStream, outStream: TStream; var fileName,
  comment: RawByteString; var dateTime: TDateTime);
const
  bufferSize = 32768;
var
  header     : TGZHeader;
...
function GZDecompressStreamSize(inStream: TStream; var fileName,
  comment: RawByteString; var dateTime: TDateTime): Longint;
const
  bufferSize = 32768;
var
...
procedure GZCompressFile(const inFileName, outFileName: String;
  const fileName, comment: RawByteString);
var
...
procedure GZCompressFile(const inFileName, outFileName: String;
  const comment: RawByteString);
var
  fileName: string;
begin
  fileName := ExtractFilename(inFileName);

  GZCompressFile(inFileName, outFileName, RawByteString(fileName), '');
end;
...
function GzipCompress(const buf; len: Integer): RawByteString;
var
...
begin
  ...
  header.Time := DateTimeToUnix(Now);
  ...
  trailer.Crc := ZLibExApi.crc32(0, buf, len);
  trailer.Size := len;

  ms := TMemoryStream.Create;
  ms.Size := SizeOf(header) + SizeOf(trailer) + len * 2;

  FillChar(zlibStrm, SizeOf(zlibStrm), 0);
  zres := deflateInit2(zlibStrm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY);

  if zres <> Z_OK then Exit;
  ...
  try
    ...
    zlibStrm.next_out := PByte(PAnsiChar(ms.Memory) + SizeOf(header));
    zlibStrm.avail_out := len * 2;
    zres := deflate(zlibStrm, Z_FINISH);

    if zres in [Z_OK, Z_STREAM_END] then
    begin
      ms.Seek(len * 2 - Integer(zlibStrm.avail_out), soFromCurrent);
      ms.WriteBuffer(trailer, SizeOf(trailer));
      SetString(Result, PAnsiChar(ms.Memory), ms.Position);
    end;
  finally
    ms.Free;
    deflateEnd(zlibStrm);
...
function GzipCompress(const s: RawByteString): RawByteString;
begin
  Result := GzipCompress(Pointer(s)^, Length(s));
end;

function GzipDecompress(const buf; len: Integer): RawByteString;
var
...
begin
  ...
  try
    ...
    begin
      ...
      if zres in [Z_OK, Z_STREAM_END] then
        ms.WriteBuffer(dest, SizeOf(dest) - zlibStrm.avail_out)
      else
        raise Exception.Create('Ω‚—π¥ÌŒÛ');
    end;
...
function GzipDecompress(const s: RawByteString): RawByteString;
begin
  Result := GzipDecompress(Pointer(s)^, Length(s));
end;
...
constructor TGZDecompressionStream.Create(source: TStream);
  ...
  function ReadNullString: RawByteString;
  var
    c: AnsiChar;
  begin
    result := '';
